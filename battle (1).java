	...
	public static void main(String[] args){
		...
		while(battleRunning){
			...
			while(stillChoosing){
				...
				c.printOut();
				System.out.println("-------------------------------------------------------------");
				System.out.println("num * 10 to get 10 times as much");
				...
				if(choice == 1 && gold >= 3 ){
					...
					fightGo = false;
					battleRunning = false;
				} else if(choice == 10 && gold >= 30){
					for(int asdf = 0; asdf < 10 ;asdf++){
						gold = i.gainPeople(gold);
					}
				} else if(choice == 20 && gold >= 40){
					for(int qw = 0; qw < 10; qw++){
						gold = a.gainPeople(gold);
					}
				} else if(choice == 30 && gold >= 50){
					for(int fghj = 0; fghj < 10; fghj++){
						gold = c.gainPeople(gold);
					}
				} else if(choice == 100 && gold >= 300){
					for(int we = 0; we < 100; we ++){
						gold = i.gainPeople(gold);
					}
				} else if(choice == 200 && gold >= 400){
					for(int rt = 0; rt < 100; rt++){
						gold = a.gainPeople(gold);
					}
				} else if(choice == 300 && gold >= 500){
